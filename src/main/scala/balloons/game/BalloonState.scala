 ...
 */
case class BalloonState(balloonIndex: Int, currentSize: Int, threshold: BalloonThreshold, powerModeEnabled: Boolean) {
  override def toString = s"Balloon #$balloonIndex $currentSize/$threshold"

  def score = if (powerModeEnabled) currentSize * 2 else currentSize
   ...
   */
  def next(size: Int): BalloonState = {
    copy(balloonIndex = balloonIndex + 1, 0, size, powerModeEnabled = false)
  }
